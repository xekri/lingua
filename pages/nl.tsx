...
Zij zijn begiftigd met verstand en geweten, en behoren zich jegens elkander in een geest van broederschap te gedragen.`
    ...
    <div className='tables'>
      <table>
        ...
        <tr>
          <th>nominative</th>
          {'ik hij zij het wij jij zij'.split(' ').map(w => <td>{convert(w)}</td>)}
        </tr>
        <tr>
          <th>oblique</th>
          {'mij hem haar het ons u hen'.split(' ').map(w => <td>{convert(w)}</td>)}
        </tr>
        <tr>
          <th>posessive</th>
          {'mijn zijn haar zijn ons uw hun'.split(' ').map(w => <td>{convert(w)}</td>)}
        </tr>
        <tr>
          <th>‘be’</th>
          <td>{convert('ben')}</td>
