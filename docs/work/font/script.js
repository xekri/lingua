document.addEventListener("DOMContentLoaded", () => {
  ...
  const f = (c, [i, j]) => {
    ...
    strokes.forEach(stroke => {
      stroke.forEach(([m, n], index) => {
        const [x, y] = [
          padding + (n + (2 + marginJ) * j) / 2 * scale,
          padding + (m * height + (2 + marginI) * i) / 2 * scale]
        if(index == 0)
          context.moveTo(x, y)
        else
          context.lineTo(x, y);
  ...
  const strokesOf = {
    ...
    i: [[p.tl, p.tr], [p.cl, p.cc]],
    a: [[p.tr, p.bl], [p.tc, p.cl]],
    u: [[p.tl, p.bl], [p.tc, p.cc]],
  }
  ...
  for(const [c, d] of ["ŋ", "n", "m",  "i", "a", "u"])
    strokesOf[c.toUpperCase()] = strokesOf[c].concat([[p.bc, p.bc]])
