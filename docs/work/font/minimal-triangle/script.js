...
const charToPaths = {
  ...
  t: [[pos.cl, pos.t, pos.br], [pos.cr, pos.bc, pos.bl]],
...
document.addEventListener("DOMContentLoaded", event => {
  ...
  for (const [f, key] of [[x => x, "input"], [parseInt, "char-w"], [parseInt, "stroke-w"], [parseFloat, "space-x"], [parseFloat, "space-y"], [parseFloat, "margin"], [x => "#" + x, "color"], [x => "#" + x, "bg-color"]]) if (params[key]) document.getElementById(key).value = f(params[key]);

  render();
  updateTweet();

  for (const e of document.getElementsByClassName("trigger")) for (const f of [render, updateTweet]) e.addEventListener("input", f);
});
...
const updateTweet = () => {
  ...
  for (const key of keys) params[key] = document.getElementById(key).value;
  ...
  document.getElementById("tweet").setAttribute("href", "https://twitter.com/intent/tweet" + `?text=` + encodeURIComponent(params.input) + `&via=sumimq` + "&hashtags=minimal_triangle" + "&url=" + encodeURIComponent(document.location.protocol + "//" + document.location.host + document.location.pathname + "?" + keys.slice(0, -1).map(key => `${key}=${params[key]}`).join("&") + "&input=" + params.input.split("\n").map(encodeURIComponent).join("%0A")));
...
const render = () => {
  ...
  for (const [j, line] of lines.entries()) for (const [i, char] of line.split("").entries()) for (const path of charToPaths[char.toLowerCase()]) {
    ctx.moveTo(...coordinate(path[0][0], path[0][1], i, j, charW, margin, spaceX, spaceY));
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.lineWidth = parseInt(document.getElementById("stroke-w").value);
    ctx.strokeStyle = document.getElementById("color").value;

    for (const [x, y] of path.slice(1)) ctx.lineTo(...coordinate(x, y, i, j, charW, margin, spaceX, spaceY));
  }

  ctx.stroke();
};

const coordinate = (x, y, i, j, charW, margin, spaceX, spaceY) => [(margin + i * (1 + spaceX) + x / unit) * charW, (margin + j * (1 + spaceY) + y / unit) * charW];